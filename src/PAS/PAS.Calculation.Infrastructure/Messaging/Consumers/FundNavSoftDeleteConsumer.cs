...
using PAS.Calculation.Application.FundPerformances.Commands.ProcessFundNavDelete;
...
public sealed class FundNavSoftDeleteConsumer : BackgroundService {
    private const string QueueName = $"pas.calculation.{AssetMessagingTopology.FundNavSoftDeleteRoutingKey}";

    private readonly IConnection _connection;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<FundNavSoftDeleteConsumer> _logger;
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    public FundNavSoftDeleteConsumer(IConnection connection, IServiceScopeFactory scopeFactory, ILogger<FundNavSoftDeleteConsumer> logger) {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken) {
        ...
        consumer.ReceivedAsync += async (_, delivery) => {
            try {
                ...
                var integrationEvent = JsonSerializer.Deserialize<FundNavSoftDeleteIntegrationEvent>(body, JsonOptions);
                ...
                if (integrationEvent is null ||
                    ...
                    throw new JsonException("Le message FundNavSoftDelete est incomplet ou invalide.");
                }
                ...
                try {
                    var command = new ProcessFundNavDeleteCommand(integrationEvent.EventId, integrationEvent.FundId, integrationEvent.Date, integrationEvent.OccurredAtUtc, integrationEvent.Version);
                ...
                _logger.LogInformation("NAV supprimée pour le fonds {FundId}, date {Date}, événement {EventId}.", integrationEvent.FundId, integrationEvent.Date, integrationEvent.EventId);
    ...
    private static async Task DeclareTopologyAsync(IChannel channel, CancellationToken cancellationToken) {
        ...
        await channel.QueueBindAsync(queue: QueueName, exchange: AssetMessagingTopology.Exchange, routingKey: AssetMessagingTopology.FundNavSoftDeleteRoutingKey, cancellationToken: cancellationToken);
    }
}
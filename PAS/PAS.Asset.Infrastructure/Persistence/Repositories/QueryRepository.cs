...
namespace PAS.Asset.Infrastructure.Persistence.Repositories {
    public sealed class QueryRepository : IQueryRepository {
        ...
        public async Task<FundDto?> GetByIdAsync(Guid id, CancellationToken cancellationToken) {
            return await _context.Funds.AsNoTracking().Where(f => f.Id == id).Select(f => new FundDto(
                f.Id,
                f.Name,
                f.Isin,
                f.Currency,
                f.Status,
                f.Navs.OrderByDescending(n => n.Date).Select(n => new FundNavDto(n.Value, DateOnly.FromDateTime(n.Date))).ToList()))
